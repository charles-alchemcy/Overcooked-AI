...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.env = OverCookedEnv(
            scenario=self.cfg.env, episode_length=self.cfg.episode_length
        )
        ...
        cfg.agent.params.critic.input_dim = (
            cfg.agent.params.obs_dim + cfg.agent.params.action_dim
        )
        ...
        action_shape = [
            len(self.env_agent_types),
            cfg.agent.params.action_dim if not self.discrete_action else 1,
        ]
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps + 1:
            if done or self.step % self.cfg.eval_frequency == 0:
                ...
                self.ou_percentage = (
                    max(
                        0, self.ou_exploration_steps - (self.step - self.num_seed_steps)
                    )
                    / self.ou_exploration_steps
                )
                self.agent.scale_noise(
                    self.ou_final_scale
                    + (self.ou_init_scale - self.ou_final_scale) * self.ou_percentage
                )
            ...
            if self.step < self.cfg.num_seed_steps:
                action = np.array(
                    [self.env.action_space.sample() for _ in self.env_agent_types]
                )
                if self.discrete_action:
            ...
            if (
                ...
                and self.step >= self.agent.batch_size
            ):
